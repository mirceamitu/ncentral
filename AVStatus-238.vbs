...
Sub ObtainCarbonBlackData
    ...
    If objFSO.FileExists(ProgramFiles64 & "\Confer\RepCLI.exe") Then
        ' output.writeline "- Successfully located the 64bit RepCLI.exe tool."
        UnifiedProgramFiles = ProgramFiles64
    ElseIf objFSO.FileExists(ProgramFiles & "\Confer\RepCLI.exe") Then
        ' output.writeline "- Successfully located the 32bit RepCLI.exe tool."
        UnifiedProgramFiles = ProgramFiles
    Else
    ...
    Set oExec = WshShell.Exec(UnifiedProgramFiles & "\Confer\repcli.exe status")
    FormattedAVVersion = "Unknown"
    Do 'Because the status command outputs many lines of text, we need to use a Do/Loop command to grab all of it
        sLine = oExec.StdOut.ReadLine()
    ...
    If objFSO.FileExists(UnifiedProgramFiles & "\Confer\db_cfg") Then
        Set objFile = objFSO.GetFile(UnifiedProgramFiles & "\Confer\db_cfg")
        AVDatVersion = objFile.DateLastModified
    Else
        AVDatVersion="01/01/2001"
	   output.writeline "- The AV Definition file was not found. Odd."
